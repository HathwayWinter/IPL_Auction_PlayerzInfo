...
import {
  ...
  SlidersHorizontal, Sparkles, Ticket, Trophy, Users, X, Pencil, Trash2, LogOut
} from 'lucide-react';
import './styles.css';
...
function App() {
  ...
  function updatePlayer(id, patch) {
    if (!isAdmin) { notify('error', 'Admin access is required to change player data.'); return; }
  ...
  function addPlayer(form) {
    if (!isAdmin) { notify('error', 'Admin access is required to add players.'); return false; }
  ...
  function savePlayer(id, form) {
    if (!isAdmin) return false;
    const name = form.name.trim();
    const basePrice = Number(form.basePrice);
    if (name.length < 3 || !form.nationality.trim() || !basePrice || basePrice < 0) { notify('error', 'Enter a valid name, nationality, and base price.'); return false; }
    if (data.players.some(p => p.id !== id && p.name.toLowerCase() === name.toLowerCase())) { notify('error', 'Another player with this name already exists.'); return false; }
    updatePlayer(id, { name, role: form.role, nationality: form.nationality.trim(), basePrice, initials: name.split(' ').map(part => part[0]).join('').slice(0, 2).toUpperCase() });
    setEditPlayer(null); notify('success', `${name} details updated.`); return true;
  }

  function removePlayer(player) {
    if (!isAdmin) { notify('error', 'Admin access is required to remove players.'); return; }
    if (!window.confirm(`Remove ${player.name} from the auction pool?`)) return;
    setData(current => ({ ...current, players: current.players.filter(p => p.id !== player.id) }));
    notify('success', `${player.name} removed from the auction pool.`);
  }
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      ...
      <nav>
        ...
        {isAdmin && <NavItem icon={<ShieldCheck size={17} />} label="Admin panel" active={false} onClick={() => notify('success', 'Admin access is active.')} />}
      </nav>
      <div className="sidebar-footer"><div className="avatar">{isAdmin ? 'AK' : 'EU'}</div><div><strong>{isAdmin ? 'Admin Kumar' : 'End User'}</strong><span>{isAdmin ? 'Administrator' : 'Read-only viewer'}</span></div><span className="online" /></div>
    </aside>
    <main className="main-content">
      <header className="topbar"><div className="breadcrumbs"><span>IPL_Auction_PlayerzInfo</span><ChevronRight size={14} /><b>{activePage === 'dashboard' ? 'Overview' : activePage === 'players' ? 'Player pool' : activePage === 'auction' ? 'Auction desk' : 'Teams & purse'}</b></div><div className="top-actions"><span className="saved"><Database size={14} /> Saved locally</span>{isAdmin && <button className="icon-button" title="Reset demo data" onClick={resetDemo}><ArrowDownUp size={17} /></button>}<button className="profile-button"><span className="avatar small">{isAdmin ? 'AK' : 'EU'}</span><span>{isAdmin ? 'Admin' : 'End User'}</span></button><button className="logout-button" title="Log out" onClick={() => setSession(null)}><LogOut size={15} /></button></div></header>
      {activePage === 'dashboard' && <Dashboard totals={totals} data={data} teamStats={teamStats} onNavigate={setActivePage} onAdd={() => setShowAdd(true)} isAdmin={isAdmin} />}
      {activePage === 'players' && <PlayersPage players={filteredPlayers} query={query} setQuery={setQuery} statusFilter={statusFilter} setStatusFilter={setStatusFilter} roleFilter={roleFilter} setRoleFilter={setRoleFilter} sort={sort} setSort={setSort} onStatus={handleStatusChange} onSelect={setDetailPlayer} onEdit={setEditPlayer} onDelete={removePlayer} onAdd={() => setShowAdd(true)} isAdmin={isAdmin} />}
      {activePage === 'auction' && <AuctionDesk players={data.players} totals={totals} onStatus={handleStatusChange} onSelect={setDetailPlayer} isAdmin={isAdmin} />}
      {activePage === 'teams' && <TeamsPage teamStats={teamStats} purse={data.auction.purse} onSelect={setDetailPlayer} />}
    </main>
    {isAdmin && showAdd && <AddPlayerModal onClose={() => setShowAdd(false)} onSubmit={addPlayer} />}
    {isAdmin && editPlayer && <EditPlayerModal player={editPlayer} onClose={() => setEditPlayer(null)} onSubmit={savePlayer} />}
    {selectedPlayer && <SaleModal player={selectedPlayer} teams={teamStats} onClose={() => setSelectedPlayer(null)} onSubmit={finalizeSale} />}
    {detailPlayer && <PlayerDetailsModal player={detailPlayer} teams={data.teams} onClose={() => setDetailPlayer(null)} />}
...
function LoginScreen({ onLogin }) {
  ...
  function submit(event) { event.preventDefault(); if (mode === 'admin' && username === 'Admin' && password === 'Admin') { onLogin({ role: 'admin' }); return; } if (mode === 'user') { onLogin({ role: 'user' }); return; } setError('Admin login requires username and password: Admin / Admin.'); }
  return <div className="login-shell"><div className="login-art"><div className="brand login-brand"><div className="brand-mark"><Ticket size={19} /></div><div><strong>PlayerzInfo</strong><span>IPL auction desk</span></div></div><div className="login-art-copy"><p className="eyebrow"><span className="live-dot" /> IPL 2026 / AUCTION CONTROL</p><h1>Every player.<br /><em>One clear board.</em></h1><p>Track the auction pool, protect every bid, and keep the season moving.</p></div><div className="login-art-footer">Pilot workspace · Local demo data</div></div><div className="login-panel"><div className="login-panel-inner"><p className="eyebrow">WELCOME BACK</p><h2>Choose your access</h2><p className="login-subtitle">Select how you want to enter the auction workspace.</p><div className="login-tabs"><button className={mode === 'admin' ? 'active' : ''} onClick={() => { setMode('admin'); setError(''); }}>Admin Login</button><button className={mode === 'user' ? 'active' : ''} onClick={() => { setMode('user'); setError(''); }}>End User View</button></div>{mode === 'admin' ? <form onSubmit={submit}><label>Username<input value={username} onChange={e => setUsername(e.target.value)} placeholder="Enter username" autoComplete="username" /></label><label>Password<input value={password} onChange={e => setPassword(e.target.value)} type="password" placeholder="Enter password" autoComplete="current-password" /></label><button className="login-button" type="submit">Enter admin workspace <ChevronRight size={16} /></button></form> : <div className="user-login-card"><div className="user-login-icon"><Users size={22} /></div><strong>Read-only access</strong><p>Browse players, auction status, sold amounts, and team purse insights without management controls.</p><button className="login-button" onClick={() => onLogin({ role: 'user' })}>Continue as end user <ChevronRight size={16} /></button></div>}{error && <div className="login-error"><CircleAlert size={16} />{error}</div>}<p className="login-note"><ShieldCheck size={14} /> Admin controls are protected for the pilot.</p></div></div></div>;
}
...
function PlayerDetailsModal({ player, teams, onClose }) { const team = teams.find(item => item.id === player.teamId); return <Modal title={player.name} subtitle="Player profile and auction record." onClose={onClose}><div className="details-card"><div className="player-avatar large">{player.initials}</div><div><strong>{player.role}</strong><span>{player.nationality} · {player.matches} IPL matches</span></div><span className={`status-pill ${player.status}`}>{statusLabel(player.status)}</span></div><div className="details-grid"><div><span>Base price</span><b>{money(player.basePrice)}</b></div><div><span>Final price</span><b>{money(player.soldAmount)}</b></div><div><span>Winning team</span><b>{team ? team.name : 'Not assigned'}</b></div><div><span>Player rating</span><b>{player.rating} / 100</b></div></div><div className="modal-actions"><button className="primary-button" onClick={onClose}>Close</button></div></Modal>; }

function EditPlayerModal({ player, onClose, onSubmit }) { const [form, setForm] = useState({ name: player.name, role: player.role, nationality: player.nationality, basePrice: String(player.basePrice) }); const update = event => setForm({ ...form, [event.target.name]: event.target.value }); return <Modal title={`Edit ${player.name}`} subtitle="Update player information and base price." onClose={onClose}><form onSubmit={event => { event.preventDefault(); onSubmit(player.id, form); }}><label>Player name<input name="name" value={form.name} onChange={update} required minLength="3" /></label><div className="form-two"><label>Role<select name="role" value={form.role} onChange={update}><option>Batter</option><option>Bowler</option><option>All-rounder</option><option>Wicketkeeper</option></select></label><label>Nationality<input name="nationality" value={form.nationality} onChange={update} required /></label></div><label>Base price<div className="input-prefix"><span>₹</span><input name="basePrice" type="number" min="0.5" step="0.5" value={form.basePrice} onChange={update} required /><em>Cr</em></div></label><div className="modal-actions"><button type="button" className="secondary-button" onClick={onClose}>Cancel</button><button type="submit" className="primary-button"><Check size={16} /> Save changes</button></div></form></Modal>; }
